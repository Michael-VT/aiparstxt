...
func process(text string) (string, map[rune]int) {
...
func buildReport(inputFile, outputFile string, replaced map[rune]int, wordFreq *map[string]int, elapsed time.Duration) string {
	var lines []string
...
func main() {
	...
	if !*noReport {
		rpt := buildReport(inputPath, outFile, replaced, wfPtr, elapsed)
	...
	fmt.Printf("Done in %.6fs. Replacements: %d\n", elapsed.Seconds(), total)
}
