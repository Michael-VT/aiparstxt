 ...
 * - Probability-based AI detection scoring (v0.4.0 canonical weights)
 * - Advanced forensic reporting with located evidence
 */
...
function buildReport(inputFile, outputFile, replaced, watermarkRemoved, wordFreq, elapsed, aiMetrics, aiResult, aiEvidence, lang = "Bun-Ext") {
  const lines = [];
  ...
  if (aiMetrics && aiResult) {
    ...
    const { verdict, interpretations } = getInterpretation(aiMetrics, aiResult.probability);
    ...
    lines.push(`  Sentence length CV:    ${aiMetrics.burstiness.toFixed(3)}`);
    const paraCv = aiMetrics.paragraph_uniformity_cv;
    lines.push(
      paraCv !== null && paraCv !== undefined
        ? `  Paragraph length CV:   ${paraCv.toFixed(3)}`
        : "  Paragraph length CV:   n/a (<4 paragraphs)"
    );
    ...
    lines.push(`  Connective density:    ${aiMetrics.connective_density.toFixed(3)}`);
    const tmpl = aiMetrics.template_header_repetition || {};
    lines.push(
      `  Template headers:      ${tmpl.total || 0} repeats (${tmpl.distinct || 0} distinct)`
    );
    ...
    const tiers = aiMetrics.ai_phrase_tiers;
    if (tiers) {
      lines.push(`  AI phrases (tiers):    high=${tiers.high}, medium=${tiers.medium}, weak=${tiers.weak}`);
    }
    ...
    if (aiEvidence && aiEvidence.length > 0) {
      lines.push("AI EVIDENCE (locations in the text):");
      aiEvidence.slice(0, 15).forEach((ev, i) => {
        const loc = ev.line ? `line ${ev.line}` : "text-wide";
        lines.push(`  [${i + 1}] ${loc}: ${ev.detail}`);
        if (ev.excerpt) {
...
function main() {
  ...
  if (cleaned && cleaned.length > 0) {
    ...
    aiMetrics = calculateAIForensicMetrics(text);
    if (aiMetrics) {
      aiResult = calculateAIProbability(aiMetrics);
      aiEvidence = buildEvidence(text, aiMetrics);
    }
  }
  ...
  if (aiResult) {
    ...
    if (aiEvidence && aiEvidence.length > 0) {
      console.log(`AI Evidence (top ${Math.min(3, aiEvidence.length)} of ${aiEvidence.length}):`);
      for (const ev of aiEvidence.slice(0, 3)) {
        const loc = ev.line ? `line ${ev.line}` : "text-wide";
        console.log(`  ${loc}: ${ev.detail}`);
      }
    }
  }
